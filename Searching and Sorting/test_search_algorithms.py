...
class TestSearchAlgorithms(unittest.TestCase):
     ...
     def _test_search_algorithm(self, search_func: 'function', input_array: list, saught_value: int, expected_output: int):
         self.assertEqual(search_func(
              input_array, saught_value), expected_output)
     ...
     def _test_all_search_funcs(self, input_array, saught_value):
          ...
          self._test_search_algorithm(
               linear_search, input_array, saught_value, expected_output)
          self._test_search_algorithm(
               binary_search_v1, input_array, saught_value, expected_output)
          self._test_search_algorithm(
               binary_search_v2, input_array, saught_value, expected_output)
          self._test_search_algorithm(
               binary_search_v3, input_array, saught_value, expected_output)
          #self._test_search_algorithm(binary_search_v4, input_array, saught_value, expected_output) # doesn't work if the value isn't in the array
          self._test_search_algorithm(
               binary_search_check_someone, input_array, saught_value, expected_output)
